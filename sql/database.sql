...
(
    ...
    resolved_report_json  text          not null comment '哼哈官汇报解析结果，以Json格式保存',
    total_used            smallint      not null default 0 comment '哼哈词总使用次数',
    used_info_per_guest   varchar(1024) not null default '' comment '每位嘉宾哼哈词使用情况',
    used_word_and_count   varchar(1024) not null default '' comment '哼哈词及使用次数映射',
