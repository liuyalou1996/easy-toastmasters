...
(
    ...
    most_used_word_top3   varchar(256)  default ''                not null comment '哼哈词使用频率最高前3，以JSON格式保存',
    used_guest_and_count  varchar(256)  default ''                not null comment '哼哈词使用嘉宾和次数，以JSON格式保存',
    most_used_guest_top3  varchar(256)  default ''                not null comment '哼哈词使用次数最多嘉宾前3，以JSON格式保存',
